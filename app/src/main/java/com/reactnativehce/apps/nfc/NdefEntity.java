...
public class NdefEntity {
  ...
  public NdefEntity(String type, String content) {
    ...
    } else if (type.startsWith("mime:") && type.length() > "mime:".length()) {
      final String mime = type.substring("mime:".length());
      record = NdefRecord.createMime(mime, content.getBytes(StandardCharsets.UTF_8));
    } else {
  ...
  public static NdefEntity fromBytes(byte[] bytes) throws Exception {
    ...
    if (record.getTnf() == NdefRecord.TNF_WELL_KNOWN && Arrays.equals(record.getType(), NdefRecord.RTD_TEXT)) {
      ...
      String content = new String(Arrays.copyOfRange(payload, 3, payload.length));
      return new NdefEntity("mime:" + Arrays.toString(record.getType()), content);
    }
